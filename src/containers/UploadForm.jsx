...
export const UploadForm = () => {
  const { 
    ...
    productImages,
    categoriesCheckedList } = useContext(DbContext)
    console.log(productName)
    console.log(sizeCheckedList)
    console.log(categoriesCheckedList)
  ...
  return (
    <div className={'w-full md:w-1/2 bg-slate-200 border-2 border-slate-400 shadow-2xl shadow-slate-600 rounded-lg'}>
      <div className='flex flex-col gap-2 p-4'>
        ...
        <input onChange={productDescriptionHandler} className='border-2 border-slate-600 p-1 rounded-lg' type='text' name='productDescription' id='productDescription' />
        ...
        {categories.map(catg => 
          <label key={catg.id} htmlFor='categories'>
            <input onChange={productCategoriesHandler} type='checkbox' className='appearance-none bg-white border border-slate-400 rounded-full w-4 h-4 checked:bg-emerald-500 checked:border-emerald-500' name='categories' id='categories' value={catg.name} />
            <span className='ml-2'>{catg.name}</span>
          </label>)}
        </div>
        <label htmlFor='productImage'>Subir archivo</label>
        <input onChange={productImagesHandler} className='border-2 border-slate-600 p-1 rounded-lg' type='file' name='productImage' id='productImage' />
